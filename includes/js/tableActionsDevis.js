...
jQuery(document).ready(function(){
    $(document).delegate('.Daction select', 'change', function(){
        ...
        var etat = $(this).closest('tr').find('td:eq(8)').text();
        ...
        if(etat == 'Fermé'){
            ...
            $(document).delegate("#Roui, #Rnon", 'click', function(){
                if($(this).val() == 'Roui'){
                    $( "#myTable tr .Did:contains("+id+")" ).closest('tbody tr').find('td:eq(8)').text('En Cours');
                    $('#demandeFermee').css('display', 'none');
                    $( "#myTable tr .Did:contains("+id+")" ).closest('tr').removeClass("Ferme");
                    conditionalColor();
                    jQuery.ajax({
                        ...
                        $(document).delegate('#oui, #non', 'click', function(){
                            if($(this).val() == 'oui'){
                                jQuery.ajax({
                                    ...
                                    success:function(){
                                        $( "#myTable tr .Did:contains("+id+")" ).closest('tbody tr').find('td:eq(8)').text('Fermé');
                                        $( "#myTable tr .Did:contains("+id+")" ).closest('tbody tr').addClass('Ferme');
    ...
    $(document).delegate('#btnenvoi', "click", function (e) {
        ...
        $( "#myTable tr .Did:contains("+id+")" ).closest('tbody tr').find('td:eq(8)').text('Transfert');
        $( "#myTable tr .Did:contains("+id+")" ).closest('tbody tr').find('td:eq(5)').text(salarie);
        $('#selected').removeAttr('id');
        $.ajax({
            url: "/wp-content/plugins/gestion_contacts/ajaxHandler.php",
            type: "post",
    ...
    $(document).delegate('#mail_submit', 'click', function(){
        ...
        var id = $('#demId').val();
        $( "#myTable tr .Did:contains("+id+")" ).closest('tbody tr').find('td:eq(8)').text('Répondu');
    ...
    $(document).delegate('#myTable tbody td:nth-child(6)', 'mouseenter', function () {

        $(this).css( 'cursor', 'pointer' );

    });
    $(document).delegate('#myTable tbody td:nth-child(6)', 'mouseleave', function () {

        $(this).css( 'cursor', 'default' );

    });
    $(document).delegate('#myTable tbody td:nth-child(6)', 'click', function(event){
